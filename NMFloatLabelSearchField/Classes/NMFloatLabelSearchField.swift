...
@IBDesignable
open class NMFloatLabelSearchField: SkyFloatingLabelTextField {
    ////////////////////////////////////////////////////////////////////////
    // Public interface
    ...
    /// Set an array of NMFloatLabelSearchFieldItem's to be used for suggestions
    open func filterItems(_ items: [NMFloatLabelSearchFieldItem]) {
        filterDataSource = items
    }
    
    /// Set an array of strings to be used for suggestions
    open func filterStrings(_ strings: [String]) {
        var items = [NMFloatLabelSearchFieldItem]()
        
        for value in strings {
            items.append(NMFloatLabelSearchFieldItem(title: value))
        }
    ...
    open var itemSelectionHandler: NMFloatLabelSearchFieldItemHandler?
    ...
    fileprivate var filterDataSource = [NMFloatLabelSearchFieldItem]() {
        didSet {
    ...
    override open func willMove(toSuperview newSuperview: UIView?) {
        ...
        self.addTarget(self, action: #selector(NMFloatLabelSearchField.textFieldDidChange), for: .editingChanged)
        self.addTarget(self, action: #selector(NMFloatLabelSearchField.textFieldDidBeginEditing), for: .editingDidBegin)
        self.addTarget(self, action: #selector(NMFloatLabelSearchField.textFieldDidEndEditing), for: .editingDidEnd)
        self.addTarget(self, action: #selector(NMFloatLabelSearchField.textFieldDidEndEditingOnExit), for: .editingDidEndOnExit)
        
        NotificationCenter.default.addObserver(self, selector: #selector(NMFloatLabelSearchField.keyboardWillShow(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(NMFloatLabelSearchField.keyboardWillHide(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(NMFloatLabelSearchField.keyboardDidChangeFrame(_:)), name: UIResponder.keyboardDidChangeFrameNotification, object: nil)
    }
    ...
    @objc open func textFieldDidChange() {
        ...
        timer?.invalidate()
        timer = Timer.scheduledTimer(timeInterval: typingStoppedDelay, target: self, selector: #selector(NMFloatLabelSearchField.typingDidStop), userInfo: self, repeats: false)
...
extension NMFloatLabelSearchField: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: NMFloatLabelSearchField.cellIdentifier)
        
        if cell == nil {
            cell = UITableViewCell(style: .subtitle, reuseIdentifier: NMFloatLabelSearchField.cellIdentifier)
        }
...
public struct NMFloatLabelSearchFieldTheme {
    ...
    public static func lightTheme() -> NMFloatLabelSearchFieldTheme {
        return NMFloatLabelSearchFieldTheme(cellHeight: 30, bgColor: UIColor (red: 1, green: 1, blue: 1, alpha: 0.6), borderColor: UIColor (red: 0.9, green: 0.9, blue: 0.9, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 10), fontColor: UIColor.black)
    }
    
    public static func darkTheme() -> NMFloatLabelSearchFieldTheme {
        return NMFloatLabelSearchFieldTheme(cellHeight: 30, bgColor: UIColor (red: 0.8, green: 0.8, blue: 0.8, alpha: 0.6), borderColor: UIColor (red: 0.7, green: 0.7, blue: 0.7, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 10), fontColor: UIColor.white)
    }
}

////////////////////////////////////////////////////////////////////////
// Filter Item
open class NMFloatLabelSearchFieldItem {
    // Private vars
...
public typealias NMFloatLabelSearchFieldItemHandler = (_ filteredResults: [NMFloatLabelSearchFieldItem], _ index: Int) -> Void
